...
class LinkedInMetricsService {
  ...
  async getComprehensiveMetrics(userEmail, period = 'month') {
    try {
      ...
      // Calculate engagement metrics from mock posts
      const totalLikes = topPosts.reduce((sum, p) => sum + parseInt(p.likes), 0);
      const totalComments = topPosts.reduce((sum, p) => sum + parseInt(p.comments), 0);
      const totalShares = topPosts.reduce((sum, p) => sum + parseInt(p.shares), 0);
      const totalReach = topPosts.reduce((sum, p) => sum + parseFloat(p.reach) * 1000, 0);
